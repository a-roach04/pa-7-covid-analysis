...
state_totals |> 
  ...
  slice(1:10) |> 
  ggplot() + 
  geom_col(aes(x = total, y = fct_reorder(state, total), fill = state)) +
  labs(x = "Total number of COVID-19 cases", y = "State") + 
  guides(fill = "none") + 
  theme_bw()
```
...
> No, it should be based on the population of each state
...
```{r}
state_totals |> 
  left_join(us_pop, join_by("state" == "NAME")) |>
  drop_na() |>
  mutate(percentage = ((total / estimate) * 100)) |>
  arrange(desc(percentage)) |>
  slice(1:10) |>
  ggplot() + 
  geom_col(aes(x = percentage, y = fct_reorder(state, percentage), fill = state)) +
  labs(x = "Percentage of COVID-19 cases", y = "State") + 
  guides(fill = "none") + 
  theme_bw()
```
...
> This data relies on people testing whenever they are sick and reporting a positive test. In some states, the COVID tests may not be accessible and/ or testing may not have been encouraged.
...
> I would be curious to see how many positive tests were from at-home, hospital, or pop-up tests. In Ithaca, our local nurses set up a drive through testing site so the one hospital in the area wouldn't be overloaded with patients who just needed a test.   
